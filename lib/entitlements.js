...
async function assignPlan(firebaseUid, plan, endsAt, source = 'ADMIN', note = '') {
  ...
  // Older installations may have marked trial_started_at as NOT NULL. Do not
  // write null lifecycle values; inactive lifecycle fields are ignored based
  // on the selected plan and can be cleared by migration 009.
  const updates = { plan: normalized };
  if (normalized === 'PRO' || normalized === 'PREMIUM') {
    updates.subscription_started_at = now.toISOString();
    updates.subscription_ends_at = expiry;
  }
  if (normalized === 'TRIAL') {
    updates.trial_started_at = now.toISOString();
    updates.trial_ends_at = expiry;
  }

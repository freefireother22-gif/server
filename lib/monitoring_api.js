...
async function upsertDevice(decoded, body, online = true) {
  ...
  const existingLookup = await getSupabase()
    .from('device_registry')
    .select('*')
    .eq('device_id', deviceId)
    .maybeSingle();
  if (existingLookup.error) throw new Error(`Device lookup failed: ${existingLookup.error.message}`);
  const existingDevice = existingLookup.data || null;
  ...
  if (role === 'child' && body.pairingId) {
    ...
    if (!ownerFirebaseUid && existingDevice?.role === 'child') {
      ownerFirebaseUid = existingDevice.owner_firebase_uid || null;
    }
  }
  const suppliedDeviceName = body.deviceName == null
    ? ''
    : String(body.deviceName).trim();
  const row = {
    device_id: deviceId,
    firebase_uid: decoded.uid,
    owner_firebase_uid: ownerFirebaseUid || existingDevice?.owner_firebase_uid || null,
    role: role === 'unknown' && existingDevice?.role ? existingDevice.role : role,
    pairing_id: body.pairingId
      ? String(body.pairingId)
      : (existingDevice?.pairing_id || null),
    // Monitoring snapshots omit optional metadata. Never replace a valid
    // Realme/Pixel model with null, which made the Parent UI say Child phone.
    device_name: suppliedDeviceName || existingDevice?.device_name || null,
    app_version: body.appVersion
      ? String(body.appVersion)
      : (existingDevice?.app_version || null),
    platform: body.platform
      ? String(body.platform)
      : (existingDevice?.platform || 'android'),
    is_online: online,
    battery_percent: body.batteryPercent != null && Number.isFinite(Number(body.batteryPercent))
      ? Number(body.batteryPercent)
      : (existingDevice?.battery_percent ?? null),
    is_charging: typeof body.isCharging === 'boolean'
      ? body.isCharging
      : (existingDevice?.is_charging ?? null),
...
async function syncSnapshot(decoded, body) {
  ...
  if (usage.length) {
    ...
    const usageDates = [...new Set(rows.map(row => row.usage_date))];
    let deleteQuery = db.from('app_usage_daily').delete()
      .eq('pairing_id', pairingId)
      .eq('child_device_id', childDeviceId);
    deleteQuery = usageDates.length === 1
      ? deleteQuery.eq('usage_date', usageDates[0])
      : deleteQuery.in('usage_date', usageDates);
    const removed = await deleteQuery;
    if (removed.error) throw new Error(`Usage refresh failed: ${removed.error.message}`);

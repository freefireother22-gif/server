...
async function upsertDevice(decoded, body, online = true) {
  ...
  if (role === 'child' && body.pairingId) {
    ...
    if (!ownerFirebaseUid) {
      ...
      if (pairing.exists) {
        ownerFirebaseUid = pairing.data()?.parentUid || null;
      }
    }
  }
  const row = {
...
async function listOwnedDevices(decoded, url) {
  ...
  const applyActivityFilter = (query) => activeOnly
    ? query.eq('is_online', true).gte('last_seen_at', activeSince)
    : query;
  const [parents, children] = await Promise.all([
    applyActivityFilter(db.from('device_registry').select('*').eq('firebase_uid', decoded.uid).eq('role', 'parent').order('last_seen_at', { ascending: false })),
    applyActivityFilter(db.from('device_registry').select('*').eq('owner_firebase_uid', decoded.uid).eq('role', 'child').order('last_seen_at', { ascending: false }))
  ]);

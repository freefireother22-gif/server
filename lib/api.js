...
const { handlePairingApi } = require('./pairing_api');
...
async function handleApi(req, res, url) {
  ...
  if (await handlePairingApi(req, res, url)) return true;
  ...
  try {
    ...
    if (req.method === 'PATCH' && match) {
      ...
      if (endsAt) {
        const rawEndsAt = String(endsAt).trim();
        if (/^\d+(\.\d+)?$/.test(rawEndsAt)) {
          const days = Number(rawEndsAt);
          if (!Number.isFinite(days) || days <= 0) {
            const error = new Error('endsAt days must be greater than 0');
            error.statusCode = 400;
            throw error;
          }
          endsAt = new Date(Date.now() + days * 24 * 60 * 60 * 1000).toISOString();
        } else {
          const parsedEndsAt = new Date(rawEndsAt);
          if (Number.isNaN(parsedEndsAt.getTime())) {
            const error = new Error('endsAt must be a number of days or a valid ISO date/time');
            error.statusCode = 400;
            throw error;
          }

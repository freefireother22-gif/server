...
async function handleControlPanelApi(req, res, url) {
  ...
  try {
    ...
    if (req.method === 'POST' && path === '/api/v1/admin/releases') {
      ...
      const row = {
        app_type: String(body.appType || 'ALL').toUpperCase(),
    ...
    if (req.method === 'POST' && path === '/api/v1/admin/campaigns') {
      ...
      const row = {
        ...
        // The Supabase check constraint accepts the final campaign state,
        // while the old "sending" value is rejected by this schema.
        status: 'sent',

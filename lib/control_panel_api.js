...
async function handleControlPanelApi(req, res, url) {
  ...
  const supported = path === '/api/v1/admin/devices' || path === '/api/v1/admin/pairings' || path === '/api/v1/admin/releases' || path === '/api/v1/admin/campaigns' || path === '/api/v1/admin/settings/maintenance' || path === '/api/v1/admin/monitoring' || /^\/api\/v1\/admin\/campaigns\/[^/]+$/.test(path);
  ...
  try {
    ...
    if (req.method === 'POST' && path === '/api/v1/admin/releases') {
      ...
      const row = { version_name: String(body.versionName), version_code: versionCode, download_url: downloadUrl.toString(), release_notes: String(body.releaseNotes || ''), minimum_supported_version_code: body.mandatory ? versionCode : Number(body.minimumSupportedVersionCode || 0) || null, mandatory: Boolean(body.mandatory), is_active: true, published_by: decoded.uid, published_at: new Date().toISOString() };
